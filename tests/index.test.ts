...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Augustus({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'AUGUSTUS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
...
describe('idempotency', () => {
  test.skip('key can be set per-request', async () => {
    const client = new Augustus({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
