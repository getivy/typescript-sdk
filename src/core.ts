...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from '@augustus/typescript-sdk'`:
   * - `import '@augustus/typescript-sdk/shims/node'` (if you're running on Node)
   * - `import '@augustus/typescript-sdk/shims/web'` (otherwise)
   */
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null) throw new AugustusError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`Augustus:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new AugustusError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
